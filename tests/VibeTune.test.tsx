...
describe('VibeTune App', () => {
  ...
  describe('App Branding', () => {
    ...
    it('shows loading state with VibeTune branding', async () => {
      ...
      vi.mocked(AuthService.getCurrentSession).mockImplementation(
        () => new Promise(resolve => setTimeout(() => resolve(null), 100))
      );
      
      render(<App />);
      
      expect(screen.getByText('Loading VibeTune...')).toBeInTheDocument();
    });
  });
  ...
  describe('Level Selection and Placement Test', () => {
    ...
    it('prominently displays self-selection with small placement test button', async () => {
      ...
      await waitFor(() => {
        const selfSelectButtons = screen.getAllByText(/Beginner|Intermediate|Advanced/);
        const placementButton = screen.getByText('Take Placement Test');
  ...
  describe('Main Chat Interface', () => {
    ...
    it('shows main app with prosody features', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByText('VibeTune')).toBeInTheDocument();
        expect(screen.getByText(/AI Prosody Practice/)).toBeInTheDocument();
      });
    });
    ...
    it('displays redo placement test button only for users who completed it', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByText('Redo Placement Test')).toBeInTheDocument();
      });
    });
    ...
    it('shows recording controls with pulsing animation', async () => {
      ...
      await waitFor(() => {
        ...
        expect(recordButton.closest('div')).toHaveClass('animate-pulse');
      });
    });
  });
          ...
          highlights: [
            {
              text: 'store',
              type: 'error',
              feedback: 'Try emphasizing "store" with rising intonation'
            }
          ],
          suggestions: ['Vary your pitch more throughout the sentence']
        }
      };
  ...
  describe('Color Scheme and UI', () => {
    it('uses VibeTune color scheme', async () => {
      ...
      await waitFor(() => {
        ...
        const elements = document.querySelectorAll('[class*="bg-accent"]');
  ...
  describe('User Flow Compliance', () => {
    it('requires registration before feature access', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByText('Master English Prosody with AI')).toBeInTheDocument();
        expect(screen.queryByText('AI Prosody Practice')).not.toBeInTheDocument();
      });
    });
  ...
  describe('Audio Analysis API', () => {
    it('processes audio with prosody feedback', async () => {
      ...
      const response = await fetch('/api/analyze-audio', {
        ...
        body: JSON.stringify({ text: 'This is important', level: 'Intermediate' })
      });
