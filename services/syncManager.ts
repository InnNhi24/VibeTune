...
export class SyncManager {
  ...
  static enableAutoSync() {
    if (this.autoSyncEnabled || typeof window === 'undefined') {
      console.log('Auto-sync already enabled or not in browser environment');
      return; // Already enabled or not in browser environment
    }

    this.autoSyncEnabled = true;
    console.log('Auto-sync enabled for authenticated user');

    // Sync when coming online
    const handleOnline = () => {
      console.log('Device came online, checking authentication before sync...');
      // Wait a moment for potential auth refresh, then sync
      setTimeout(() => {
