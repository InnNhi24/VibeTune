  ...
  static async flush() {
    ...
    try {
      ...
      if (error) {
        ...
        // Store failed events offline for retry
        eventsToInsert.forEach(event => this.storeOfflineEvent(event));
      } else {
        console.log(`Successfully tracked ${eventsToInsert.length} events`);
      }
  ...
  private static async syncOfflineEvents() {
    try {
      ...
      for (let i = 0; i < eventsToInsert.length; i += batchSize) {
        ...
        if (error) {
          console.error('Failed to sync offline analytics batch:', error);
